...
import { AssetHeader, AssetHeaderSkeleton } from "@/components/asset/asset-header"
import { ThesisStrip, ThesisStripSkeleton } from "@/components/asset/thesis-strip"
...
export default function AssetPage() {
  ...
  if (!assetPending && asset === null) {
    return (
      <NotFoundPanel
        message={`We couldn't find the asset "${ticker.toUpperCase()}".`}
        backHref="/assets/nvda"
        backLabel="Back to NVDA"
      />
    )
  }
  ...
  return (
    <div className="flex flex-1 flex-col">
      {asset ? <AssetHeader asset={asset} /> : <AssetHeaderSkeleton />}
      {content ? <ThesisStrip theses={content.theses} /> : <ThesisStripSkeleton />}
      ...
      <div className="mx-auto w-full max-w-[100rem] flex-1 px-4 py-6 xl:grid xl:grid-cols-[minmax(0,1fr)_400px] xl:items-start xl:gap-8 2xl:px-8">
        <main className="flex min-w-0 flex-col gap-8">
          ...
          <section id="theses" className="flex scroll-mt-4 flex-col gap-3">
            <h2 className="text-sm font-medium text-muted-foreground">
              Theses{content ? ` (${content.theses.length})` : ""}
            </h2>
